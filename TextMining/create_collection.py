...
def collection(listOfdata,filename="Collection.csv"):
    ...
    for i in range(0,len(listOfdata)):
        if listOfdata[i][0]=='T':
            patt_theorem=r'Theorem\s[0-9]+\.[0-9]?\.?\s\(?[a-zA-Z]+\s?-?[a-zA-Z]+\)?|Theorem\s[0-9]+\.[0-9]?\.?'
            ...
            patt_def=r'Definition\s[0-9]+\.[0-9]?\.?\s\(?[a-zA-Z]+\s?-?[a-zA-Z]+\)?|Definition\s[0-9]+\.[0-9]?\.?'
            ...
            patt_lemma=r'Lemma\s[0-9]+\.[0-9]?\.?\s\(?[a-zA-Z]+\s?-?[a-zA-Z]+\)?|Lemma\s[0-9]+\.[0-9]?\.?'
