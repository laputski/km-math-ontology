...
var Sum = 0;
for (var i = 0; i < Id.length; i++) {
        Sum = Sum + ChildrenId[i].length   };

var k = 0;
var Links = [];
while (k < Sum) {
   for (var i = 0; i < Id.length; i++) {
      if (ChildrenId[i].length > 0)
         for (var j = 0; j < ChildrenId[i].length; j++) {
                         Links[k] = {"source": Id[i], "target": ChildrenId[i][j]};
                                                     k++}
                                       }
                }
...
node.on("dblclick.zoom", function (d) {
    ...
    var dcx = (window.innerWidth*0.5 / 2 - d.x * zoom.scale());
    var dcy = (window.innerHeight*0.8 / 2 - d.y * zoom.scale());
